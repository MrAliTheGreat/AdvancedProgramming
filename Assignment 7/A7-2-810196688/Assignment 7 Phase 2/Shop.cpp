...
#define TRAIN_SET_ADDRESS "Data/train.csv"
#define TEST_SET_ADDRESS "Data/test.csv"
#define PROBABILITIES_ADDRESS "Data/class probabilities"
...
void Shop::signup(std::vector<std::string> command_info){
	...
	if(word_finder_in_command(command_info , TYPE) != -1){
		if(command_info[word_finder_in_command(command_info , TYPE)] == SELLER){
			...
			std::cout << SUCCESS;
		}else if(command_info[word_finder_in_command(command_info , TYPE)] == SELLER){
			id_users++;
...
void Shop::post_comment(std::vector<std::string> command_info){
	...
	std::unordered_map<std::string , double> trainset_good_info = trainset_extractor(TRAIN_SET_ADDRESS , GOOD_STATUS);
	std::unordered_map<std::string , double> trainset_bad_info = trainset_extractor(TRAIN_SET_ADDRESS , BAD_STATUS);
	double good_probability = good_probability_extractor(PROBABILITIES_ADDRESS);

	std::string comment = command_extractor(command_info , word_finder_in_command(command_info , COMMENT));
	for(int i = 0 ; i < all_products.size() ; i++){
		if(all_products[i]->get_product_id() == target_product_id && calculate_status_single_comment(comment , trainset_good_info , trainset_bad_info , good_probability) == 0){
	...
	for(int j = 0 ; j < user_cart.size() ; j++){
		for (int i = 0 ; i < all_users.size() ; i++){
			if(user_cart[j]->get_seller_id() == all_users[i]->get_user_id()){
				...
				all_users[i]->add_to_wallet(user_cart[j]->get_unit_price());
...
void Shop::evaluate_model(){
	std::unordered_map<std::string , double> trainset_good_info = trainset_extractor(TRAIN_SET_ADDRESS , GOOD_STATUS);
	std::unordered_map<std::string , double> trainset_bad_info = trainset_extractor(TRAIN_SET_ADDRESS , BAD_STATUS);
	double good_probability = good_probability_extractor(PROBABILITIES_ADDRESS);
	calculate_model_properties(TEST_SET_ADDRESS , trainset_good_info , trainset_bad_info , good_probability);
}